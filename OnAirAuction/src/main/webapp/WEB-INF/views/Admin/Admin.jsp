...
<div class="body">
	...
	<div class="form-group">
			...
			<select class="form-control" name="category" id="type">
				...
				<option value="5">회원 등급 조건</option>
	...
	<div class="form-group list">
	<table class="table table-hover condition" style="display: none;">
		<thead>
			<tr>
				<th>회원 등급 이름</th>
				<th>신고 건수</th>
		...
		<tbody>
			<c:forEach items="${rs}" var="rs" varStatus="vs">
				<tr>
					<form action="<c:url value='/admin/RatingStandard/update'></c:url>" method="post"> 
						<td>
							<input type="text" class="form-control" value="${rs.rs_ml_name}" name="rs_ml_name">
						</td>
						<td>
							<input type="number" class="form-control" value="${rs.rs_complation_number}" name="rs_complation_number">
						</td>
						<td>
							<input type="number" class="form-control" value="${rs.rs_report_number}" name="rs_report_number">
						</td>
						<td>
							<button class="btn btn-outline-warning btn-up">수정</button>
							<button class="btn btn-outline-danger move">삭제</button>
						</td>
					</form>
				</tr>
			</c:forEach>
		...
		<tfoot>
			<tr>
				<form action="<c:url value='/admin/RatingStandard/insert'></c:url>" method="post">
					<td>
						<input type="text" class="form-control" name="rs_ml_name">
					</td>
					<td>
						<input type="number" class="form-control" name="rs_complation_number">
					</td>
					<td>
						<input type="number" class="form-control" name="rs_report_number">
					</td>
					<td><button class="btn btn-outline-success">등록</button></td>
				</form>
			</tr>
...
$('.move').click(function(){
	...
	if(!res)
		return false;
	//클릭된 삭제 버튼의 조상 중에 form태그를 찾아서 action부분을 수정
	let url = '<c:url value="/admin/RatingStandard/delete"></c:url>';
	$(this).parents('tr').find('form').attr('action', url);
});
