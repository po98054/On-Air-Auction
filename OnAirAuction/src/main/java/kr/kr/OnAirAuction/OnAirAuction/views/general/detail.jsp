...
.product-info-btn-area button{
  ...
  width: 620px; height: 48px; color: #fff; font-size: 18px; border-radius: 18px;
  line-height: 48px; 
...
.product-review-area{
  display: table; table-layout: fixed; width: 100%; font-size: 13px;
  margin-top: 25px; margin-bottom: 10px;
}
.product-review-area .feedback-table{
  box-sizing: border-box; text-indent: initial;
  border-color: grey; width: 99%;
}
...
#bidlist-modal .bidlist-content .bidlist-scroll {
  max-height: 200px; /* 스크롤 가능한 최대 높이 설정 */
  overflow-y: auto; /* 세로 스크롤 표시 (필요할 때만) */
}
...
#qna-modal .qna-table .title{
  width: 500px;
  padding: 7px 18px 5px 12px; height: 30px;
    ...
    <div class="product-category">
              ...
              <div class="thumnail-swiper-wrapper">
                <c:forEach items="${prodFile}" var="prodFile">
                  <div class="swiper-slide-img">
					<img src="<c:url value="/download${prodFile.fi_save_name}"></c:url>" class="thumbnail" height="400" width="600">
                  </div>
			    </c:forEach>
              </div>
      ...
      <div class="product-tab-wrap">
        ...
        <div class="product-img-area" id="tab1">
	        <c:forEach items="${prodFile}" var="prodFile">
				<img src="<c:url value="/download${prodFile.fi_save_name}"></c:url>" height="400" width="600">
			</c:forEach>
        </div>
        <!-- 상품 설명 끝 -->
        <!-- 구매 후기 시작 -->
        ...
        <div class="product-review-area" id="tab2">
          ...
          <div class="review-layout-left">
           ...
           -->
          <div class="review-layout-right" style="margin-bottom: 10px">
          ...
          <div class="pagination-container">
           <ul class="pagination justify-content-center">
			  	<c:if test="${rePm.prev}">
			  		<li class="page-item">
			  			<a class="page-link" href="<c:url value='/general/list?page=${rePm.startPage-1}&search=${rePm.criteria.search}'></c:url>">이전</a>
			  		</li>
			  	</c:if>
			  	<c:forEach begin="${rePm.startPage}" end="${rePm.endPage }" var="i">
			  		<li class="page-item <c:if test="${rePm.criteria.page == i}">active</c:if>">
			  			<a class="page-link" href="<c:url value='/general/list?page=${i}&search=${rePm.criteria.search}'></c:url>">${i}</a>
			  		</li>
			  	</c:forEach>
			  	<c:if test="${rePm.next}">
			  		<li class="page-item">
			  			<a class="page-link" href="<c:url value='/general/list?page=${rePm.endPage+1}&search=${rePm.criteria.search}'></c:url>">다음</a>
			  		</li>
			  	</c:if>
		  	</ul>
	  	<div class="pagination-container">
                ...
                <div class="qna-content">
                  ...
                  <form action="<c:url value="/inquiry"></c:url>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="bottom-btns">
                      <a class="btn-confirm" id="btn-confirm">
                        <span>등록</span>
                      </a>
            ...
            <div class="list-qna">
              <table class="table-qna">
	            ...
	            <ul class="pagination justify-content-center">
				  	<c:if test="${inPm.prev}">
				  		<li class="page-item">
				  			<a class="page-link" href="<c:url value='/general/list?page=${inPm.startPage-1}&search=${inPm.criteria.search}'></c:url>">이전</a>
				  		</li>
				  	</c:if>
				  	<c:forEach begin="${inPm.startPage}" end="${inPm.endPage }" var="i">
				  		<li class="page-item <c:if test="${inPm.criteria.page == i}">active</c:if>">
				  			<a class="page-link" href="<c:url value='/general/list?page=${i}&search=${pm.criteria.search}'></c:url>">${i}</a>
				  		</li>
				  	</c:forEach>
				  	<c:if test="${inPm.next}">
				  		<li class="page-item">
				  			<a class="page-link" href="<c:url value='/general/list?page=${inPm.endPage+1}&search=${inPm.criteria.search}'></c:url>">다음</a>
				  		</li>
				  	</c:if>
			  	</ul>
            </div>
...
$('.btn-product-like').click(function () {
	let productLikeState = $("#productLikeState").val();
	$.ajax({
		type: 'POST',
		url: '<c:url value="/likeProduct"></c:url>',
...
$(document).ready(function(){ 
    ...
    if(productLikeState == 1){
        $('.btn-product-like i').attr('id','#sel');
    }else if(productLikeState == 0){
        $('.btn-product-like i').removeAttr('id');
    }
});
$(document).ready(function(){
    $('.product-id .btn-product-like i').click(function(){
        let identify;
        if($('.btn-product-like i').attr('id') === undefined){
            identify = true;
        }else{ 
            identify = false;
        }
...
const countDownTimer = function (id, date) {
	...
	function showRemaining() {
		var now = new Date();
		var distDt = _vDate.getTime() - now.getTime();
		...
		function hideButton(buttonId) {
			...
			if (distDt < 0) {
			  ...
			  hideButton('btn-modal-bid_open');
			  hideButton('btn-modal-imme_open');
			  return;
			}
...
//서버에서 UTC 시간대로 전달된 값
var utcDateString = '${auction.au_final_date}';
// UTC 시간대로 변환된 날짜 객체 생성
var utcDate = new Date(utcDateString + ' UTC');
// 로컬 시간대로 변환하여 사용
var localDate = utcDate.toLocaleString();

var auctionDate = new Date('${auction.au_final_date}');
countDownTimer('countDday', auctionDate);
