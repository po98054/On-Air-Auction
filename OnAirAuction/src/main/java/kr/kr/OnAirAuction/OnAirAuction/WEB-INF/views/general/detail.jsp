        ...
        <div class="product-review-area" id="tab2">
          ...
          <div class="pagination-container">
           <ul class="pagination justify-content-center">
			  	<c:if test="${rePm.prev}">
			  		<li class="page-item">
			  			<a class="page-link" href="<c:url value='/general/list?page=${rePm.startPage-1}&search=${rePm.cri.search}'></c:url>">이전</a>
			  		</li>
			  	</c:if>
			  	<c:forEach begin="${rePm.startPage}" end="${rePm.endPage }" var="i">
			  		<li class="page-item <c:if test="${rePm.cri.page == i}">active</c:if>">
			  			<a class="page-link" href="<c:url value='/general/list?page=${i}&search=${rePm.cri.search}'></c:url>">${i}</a>
			  		</li>
			  	</c:forEach>
			  	<c:if test="${rePm.next}">
			  		<li class="page-item">
			  			<a class="page-link" href="<c:url value='/general/list?page=${rePm.endPage+1}&search=${rePm.cri.search}'></c:url>">다음</a>
			  		</li>
			  	</c:if>
		  	</ul>
	  	<div class="pagination-container">
            ...
            <div class="list-qna">
              <table class="table-qna">
                ...
                <tbody>
                	<c:forEach items="${inquiry }" var="inqu">
                	  <tr>
	                    ...
	                    <td>${inqu.in_title }</td>
	                    <td>${inqu.in_me_id }</td>
	                    <td>${inqu.in_register_date_str }</td>
                      </tr>
	                </c:forEach>
                </tbody>
              </table>
	            <ul class="pagination justify-content-center">
				  	<c:if test="${inPm.prev}">
				  		<li class="page-item">
				  			<a class="page-link" href="<c:url value='/general/list?page=${inPm.startPage-1}&search=${inPm.cri.search}'></c:url>">이전</a>
				  		</li>
				  	</c:if>
				  	<c:forEach begin="${inPm.startPage}" end="${inPm.endPage }" var="i">
				  		<li class="page-item <c:if test="${inPm.cri.page == i}">active</c:if>">
				  			<a class="page-link" href="<c:url value='/general/list?page=${i}&search=${pm.cri.search}'></c:url>">${i}</a>
				  		</li>
				  	</c:forEach>
				  	<c:if test="${inPm.next}">
				  		<li class="page-item">
				  			<a class="page-link" href="<c:url value='/general/list?page=${inPm.endPage+1}&search=${inPm.cri.search}'></c:url>">다음</a>
				  		</li>
				  	</c:if>
			  	</ul>
            </div>
...
//문의하기 페이지 번호 클릭 시 번호 색상 변경
$(document).ready(function() {
  $('#btn-modal-qna_open').click(function(event) {
   alert("구현예정입니다.")
  })
});
