		...
		<tfoot >
			<tr style="margin-top: 60px;">
				  <form action="<c:url value='/AddressBook/insert'></c:url>" method="post">
					...
					<td class="form-group" style="">
						<input class="form-control" style="margin-left: 15px; width: 40%; display: inline;" placeholder="우편번호" name="bl_post_num" id="addr4" type="text" readonly="readonly" >
    					<button type="button" class="btn btn-default" onclick="PostCode();"><i class="fa fa-search"></i> 우편번호 찾기</button>
    				</td>
    				<td>
    					<input class="form-control" style="top: 5px;" placeholder="도로명 주소" name="bl_road_name" id="addr5" type="text" readonly="readonly" />
    					<input class="form-control" placeholder="상세주소" name="bl_detail_address" id="addr6" type="text"/>
					</td>
    ...
    function PostCode() {
        new daum.Postcode({
            oncomplete: function(data) {
               ...
               // 우편번호와 주소 정보를 해당 필드에 넣는다.
               document.getElementById('addr4').value = data.zonecode; //5자리 새우편번호 사용
               document.getElementById('addr5').value = fullRoadAddr;
               
               console.log(data.zonecode);
               
               console.log(fullRoadAddr);
               
               console.log(document.getElementById('addr4').value);
